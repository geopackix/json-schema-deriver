...
async function getNewSchema(name: string = "JSON File") {
  let newSchema = {
    description:
      "Auto-Generated JSON Schema for " +
      name +
...
export async function deriveSchema(inObj, name: string = "JSON File") {
  try {
    ...
    let schemaProperties = getObjectSchema(
      inObj,
...
function getObjectSchema(inObject, schemaDefinitions?: any, parent?: string) {
  ...
  if (objectSchema.type == "array") {
    ...
    if (typeof firstItem == "object") {
      ...
      let definitionName = getDefinitionByItem(
    ...
    for (let propertyName in inObject) {
      objectSchema.properties[propertyName] = getObjectSchema(
        inObject[propertyName],
        schemaDefinitions,
        propertyName
      );
    }
...
function getDefinitionByItem(item: object, schemaDefinitions: object) {
  ...
  for (let def in schemaDefinitions) {
    let defItems = getChildItemTypesByDefinition(
      schemaDefinitions[def].properties
    );
...
/**
 * Compares two Arrays (by property names and types) and returns true if they are equal .
 */
